...
// ---------- Ladder (XP / Succès) ----------
// Not wired up for Askawawa 2.73 yet (no live refresh pipeline on this server version) -
// the button/modal are kept in place for later, just showing a placeholder for now.
function openLadderModal() {
  document.getElementById("ladderModalOverlay").classList.remove("hidden");
  document.getElementById("ladderList").innerHTML = '<div class="stat-empty">Bientôt disponible.</div>';
  document.getElementById("ladderUpdated").textContent = "";
}
...
function isPaFilterItem(item) {
  // Special case per user request: "Collier de Bavdur" (+2 PA) is the one amulette allowed
  // in the PA list even though it fails the "not amulette" rule below.
  if (item.id === 22214) return true;
